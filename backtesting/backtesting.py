    ...
    def I(  # noqa: E743 N802
        ...
        **kwargs,
        ...
        if name is None:
            ...
            name = name.format(*map(_as_str, args), **dict(zip(kwargs.keys(), map(_as_str, kwargs.values()))))
...
class _Orders(tuple):
    ...
    def __getattr__(self, item):
        ...
        removed_attrs = ("entry", "set_entry", "is_long", "is_short", "sl", "tp", "set_sl", "set_tp")
        ...
        *,
        ...
        if is_long:
            if not (sl or -np.inf) < (limit or stop or adjusted_price) < (tp or np.inf):
                raise ValueError(
                    "Long orders require: " f"SL ({sl}) < LIMIT ({limit or stop or adjusted_price}) < TP ({tp})"
                )
        else:
            if not (tp or -np.inf) < (limit or stop or adjusted_price) < (sl or np.inf):
                raise ValueError(
                    "Short orders require: " f"TP ({tp}) < LIMIT ({limit or stop or adjusted_price}) < SL ({sl})"
                )
...
class Backtest:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            (data.index.is_numeric() and (data.index > pd.Timestamp("1975").timestamp()).mean() > 0.8)
        ):
            try:
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        indicator_attrs = {
            attr: indicator for attr, indicator in strategy.__dict__.items() if isinstance(indicator, _Indicator)
        }.items()

        # Skip first few candles where indicators are still "warming up"
        # +1 to have at least two entries available
        start = 1 + max(
            (np.isnan(indicator.astype(float)).argmin(axis=-1).max() for _, indicator in indicator_attrs), default=0
        )
        ...
        * `"skopt"` which finds close-to-optimal strategy parameters using
        ...
        def _optimize_skopt() -> Union[pd.Series, Tuple[pd.Series, pd.Series], Tuple[pd.Series, pd.Series, dict]]:
            try:
                ...
                raise ImportError(
                    "Need package 'scikit-optimize' for method='skopt'. " "pip install scikit-optimize"
                ) from None

            nonlocal max_tries
            max_tries = (
                200 if max_tries is None else max(1, int(max_tries * _grid_size())) if 0 < max_tries <= 1 else max_tries
            )
